...
const ProfileEditForm = ({
  ...
  const finishHandler = (value: UserEditPayload) => {
    onSubmitForm({
      ...value,
      username: userInfo && userInfo.userName ? userInfo.userName : value.username,
      profileimage: image,
    });
  };
  ...
  return (
    <Form name="profile-edit-form" onFinish={finishHandler}>
      ...
      <Form.Item label="Birth Date" name="birthdate">
        <DatePicker
          {...birthDateConfig}
          defaultValue={dayjs(new Date(userInfo ? userInfo.birthDate : new Date().getTime()))}
        />
      </Form.Item>
      ...
      <div className={styles.action}>
        <MainButton variant="primary-outline" onClick={cancelFormClickHandler}>Cancel</MainButton>
        <MainButton className={loading ? "overlay-loading" : ""} type="submit">
          Submit
        </MainButton>
      </div>
    </Form>
